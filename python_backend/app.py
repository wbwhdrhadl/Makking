...
# Function to replace swear words with '**' in the transcript
...
async def transcribe(file: UploadFile = File(...), num_speakers: int = 2):
    ...
    for i, segment in enumerate(segments):
        ...
        # Censor swear words in the transcript
        censored_text = censor_swear_words(segment["text"][1:])
        
        transcript.append({
            "start": segment["start"],
            "end": segment["end"],
            "speaker": speaker_info,
            "text": censored_text  # Use censored text
        })
